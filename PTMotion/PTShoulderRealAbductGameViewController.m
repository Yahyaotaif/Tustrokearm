...
#import "PTShoulderRealAbductGameViewController.h"
#import "pthelpers.h"
// views
#import "MonkeyView.h"
...
@interface PTShoulderRealAbductGameViewController ()
...
@implementation PTShoulderRealAbductGameViewController

#pragma mark - Override
-(void) viewDidLoad{
    
    NSLog(@"The view loaded in frotate controller");
    [[NSUserDefaults standardUserDefaults] setInteger: 8 forKey:@"exerciseID"];
...
- (void)presentUserInstructions:(BOOL)animated
{
   ...
   /* UIAlertAction *actionR = [UIAlertAction actionWithTitle:@"Ready" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        self.session.exercise = PTExerciseShoulderRealAbduction;
...
- (void)sessionStateDidChange:(PTSession*)context
{
    if (self.session.state == PTSessionStateActive) {
        [self.motionStateContext startDeviceMotionUpdatesForType:MotionStateAbduction2];
    } else if (self.session.state == PTSessionStateEnded) {
        [self.motionStateContext stopDeviceMotionUpdates];
        NSUInteger x = [[NSUserDefaults standardUserDefaults] integerForKey:@"sAcompCount"];
        x++;
        [[NSUserDefaults standardUserDefaults] setInteger: x forKey:@"sAcompCount"];
...
{
    if (self.session.state == PTSessionStateActive) {
        if ([oldState isKindOfClass:[ShoulderAbductStateRaised class]] && [newState isKindOfClass:[ShoulderAbductStateLowered class]]) {
            if (self.monitorForActionCount == NO) {
                self.monitorForActionCount = YES;
            }
        } else if ([oldState isKindOfClass:[ShoulderAbductStateLowered class]] && [newState isKindOfClass:[ShoulderAbductStateRaised class]]) {
...
{
    if ([state isKindOfClass:[ShoulderAbductStateLowered class]]) {
        ...
        NSLog(@"Shoulder adduct lowered %f",pitchAbs);
        
    } else if ([state isKindOfClass:[ShoulderAbductStateRaised class]]) {
        ...
        double pitchDegrees = pitch * 180 / 3.14;
        double pitchAbs = fabs(pitchDegrees);
        NSLog(@"Shoulder adduct raised %f",pitchAbs);
    }
}
@end
